...
####################################################################################################
# Differentiable optimization layers (nominal and distributionally robust)
####################################################################################################



class nominal(torch.autograd.Function):
    """
    ...
    def __init__(self, n_y, n_obs, prisk):
        self.n_y = n_y
        self.n_obs = n_obs
        self.prisk_type = prisk
        self.prisk = eval('rf.'+prisk)

    @staticmethod
    def forward(ctx, self, ep, y_hat, gamma):
        """
        ...
        ctx.save_for_backward(ep, y_hat, gamma)
        ...
        # Parameters
        ep = cp.Parameter((self.n_obs, self.n_y))
        y_hat = cp.Parameter(self.n_y)
        ...
        # Objective function
        objective = cp.Minimize((1/self.n_obs) * cp.sum(obj_aux) - gamma * mu_aux)
    ...
    def backward(ctx, grad_output):
        ...
        """
        ep, y_hat, gamma, = ctx.saved_tensors
